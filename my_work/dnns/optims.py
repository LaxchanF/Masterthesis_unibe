...
def objective(trial, device, model_name, train_loader, test_loaders):
    ...
    algorithm = trial.suggest_categorical("algorithm", ["Adadelta_standardino", "Adadelta_secondos", "Adadelta_optimus"])

    #Optimizer stuff
    weight_decay = trial.suggest_float("weight_decay", 1e-2, 1e2, log=True)
    lr = trial.suggest_float("lr", 1e-2, 1e3, log=True)
    rho = trial.suggest_float("rho", 1e-3, 1e1, log=True)
    eps = trial.suggest_float("eps", 1e-8, 1e-4, log=True)

    if algorithm == "Adadelta_standardino":
        optimizer = optim.Adadelta(model.parameters())
    elif algorithm == "Adadelta_secondos":
        optimizer = optim.Adadelta(model.parameters(), lr=lr, weight_decay=weight_decay)
    elif algorithm == "Adadelta_optimus":
        optimizer = optim.Adadelta(model.parameters(), lr=lr, weight_decay=weight_decay, rho=rho)
