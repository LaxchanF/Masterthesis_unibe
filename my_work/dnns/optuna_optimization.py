...
def objective(trial, device, model_name, train_loader, test_loaders):
    ...
    if algorithm == "Adadelta":
        lr_Adadelta = trial.suggest_float("lr_Adadelta", 1e-2, 1e2, log=True)
        weight_decay_Adadelta = trial.suggest_float("weight_decay_Adadelta", 1e-2, 1e2, log=True)
        optimizer = optim.Adadelta(model.parameters(), lr=lr_Adadelta, weight_decay=weight_decay_Adadelta)
    elif algorithm == "Adam":
        lr_Adam = trial.suggest_float("lr_Adam", 1e-5, 1e-1, log=True)
        weight_decay_Adam = trial.suggest_float("weight_decay_Adam", 1e-2, 1e2, log=True)
        optimizer = optim.Adam(model.parameters(), lr=lr_Adam, weight_decay=weight_decay_Adam) #weight_decay=weight_decay rausnehmen. adam sensitiv af
    elif algorithm == "AdamW":
        lr_AdamW = trial.suggest_float("lr_AdamW", 1e-5, 1e-1, log=True)
        weight_decay_AdamW = trial.suggest_float("weight_decay_AdamW", 1e-4, 1e-2, log=True)
        optimizer = optim.AdamW(model.parameters(), lr=lr_AdamW, weight_decay=weight_decay_AdamW)
    elif algorithm == "SGD":
        lr_SGD = trial.suggest_float("lr_SGD", 1e-5, 1e-1, log=True)
        weight_decay_SGD = trial.suggest_float("weight_decay_SGD", 1e-2, 1e2, log=True) 
        momentum_SGD = trial.suggest_float("momentum_SGD", 1e-2, 1e2)
        nesterov_SGD = trial.suggest_categorical("nesterov_SGD", [True, False])
        optimizer = optim.SGD(model.parameters(), lr=lr_SGD, momentum=momentum_SGD, weight_decay=weight_decay_SGD, nesterov=nesterov_SGD)
