...
# Assuming `rohdaten` is a list of data frames
daten <- map(rohdaten, \(x) {
  x %>%
    mutate(
      image_name = basename(image_name),
      angle = as.integer(str_extract(image_name, "(?<=z_)-?\\d+(?=\\.png)")),
      hit = if_else(ground_truth == prediction, 1, 0)
    ) %>%
    group_by(phase)
})

# Now group by epoch and angle, calculate accuracy and Clopper-Pearson CI
ci_data <- map(daten, \(x) {
  x %>%
    group_by(epoch, angle) %>%
    filter(epoch == 1) %>% 
    summarise(
      success = sum(hit),
      trials = n(),
    ...
    rowwise() %>%
    mutate(
      ci = list(binom.test(success, trials, conf.level = 0.95)$conf.int),
      lower = ci[[1]],
      upper = ci[[2]]
    ) %>%
    select(-ci)
...
for (i in seq_along(ci_data)) {
  architecture_df <- ci_data[[i]]
